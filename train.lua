...
require 'util.OneHot'
require 'util.misc'
local SplitMusicMMinibatchLoader = require 'util.SplitMusicMMinibatchLoader'
...
cmd:option('-model', 'grid_lstm', 'grid_lstm, lstm,gru or rnn')
...
model_filename = model_basename .. cur_iter
...
data = torch.Tensor(1,2):fill(0)
list_of_file_dirs = {}

--[[
count = 1
for file in lfs.dir(opt.dataset_directory) do
  if (string.sub(file, -3) == "mp3") then
    print(count .. ".t7")
    local timer = torch.Timer()
    table.insert(list_of_file_dirs, 'datasets/tensors/' .. count .. '.t7')
    torch.save('datasets/tensors/' .. count .. '.t7', audio.load(opt.dataset_directory .. file), 'ascii')
    count = count + 1
...
for file in lfs.dir(opt.dataset_directory) do
  if (string.sub(file, -3) == "mp3") then
    local timer = torch.Timer()
    if count <= 1 then
      data = torch.cat(data, audio.load(opt.dataset_directory .. file), 1)
      count = count + 1
    end
    print(count)
    print(timer:time().real)
...
torch.save('datasets/tensors/' .. 'data_small' .. '.t7', data[{{2,data:size(1)}}], 'ascii')

print('Loading data')
data = torch.load('datasets/tensors/data_small.t7', 'ascii')
